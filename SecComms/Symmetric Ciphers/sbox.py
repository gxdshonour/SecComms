...
def sub_bytes(state, sbox=s_box):
    ...
    # Substitute 4x4 matrix and return substituted matrix
    ...
    for state_row in range(len(state)):
        ...
        for state_col in range(len(state[state_row])):
            ...
            val = hex(state[state_row][state_col])[2:].zfill(2)

        
            s_row, s_col = int(val[0], 16), int(val[1], 16)
...
def pro_matrix2bytes(matrix):
  # Changes 4x4 into  16-byte array
